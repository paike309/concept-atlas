...
> **定位**：这是一张**结构地图**，不是事实依据。内容整理自 [`concepts/ai/`](../concepts/ai/) 下的卡片，这些卡片目前的信源等级为 `link-checked`——含义是"来源链接已机器核验（可达性 + 页面标题与卡片声称一致），但**正文表述与来源原文尚未逐条比对，数量级数字也未实测**"。见 [`verification-log.md`](verification-log.md) 与 [`round2-verification-checklist.md`](round2-verification-checklist.md)。
...
| **L3 推理** | 权重有了，但要跑得起来 | 显存、延迟、成本变成硬约束；**而且"想得更久"要另付一笔** | 8 |
| **L4 工程** | 模型能力要变成产品能力 | 引入检索、提示、选型一整条链路 | 5 |
| **L5 评估与安全** | 上面所有工作怎么算有效？ | 基准会饱和、会被污染，评估本身成为难题 | 5 |
| **L6 多模态** | 真实世界的输入不止文本 | 连续信号要离散化；对齐与生成的评估各成一套难题 | 4 |

层级分布（截至 2026-09-16）：**基础 15 / 进阶 18 / 前沿 7**。
...
| [思维链与推理模型](../concepts/ai/chain-of-thought-and-reasoning.md) | 让模型先写推理再回答——**把"一步到位的映射"摊成多步计算**。它提升能拆步骤的任务，不增加知识 | 进阶 | 指令微调与对话格式 |
| [测试时计算扩展](../concepts/ai/test-time-compute.md) | 推理时多投入计算（长思考、多采样、验证）。**收益强依赖"答案可不可验证"**；本质是把训练成本换成推理成本 | 前沿 | 思维链与推理模型 |
...
| 概念 | 起什么作用 | 层级 | 前置 |
|---|---|---|---|
| [视觉编码器](../concepts/ai/vision-encoder.md) | 把图像切成 patch 再变成向量序列。**与分词是同一个问题的另一种形态**：连续输入怎么离散化 | 进阶 | Transformer 架构 |
| [跨模态对齐](../concepts/ai/cross-modal-alignment.md) | 让图文表示落在同一语义空间，使"以文搜图"成立。**对齐的是整体语义，不是细粒度对应**——能检索 ≠ 能推理 | 前沿 | 视觉编码器 |
| [语音与音频模型](../concepts/ai/speech-and-audio-models.md) | 波形 → 频谱图 → 序列 → 文本。**转录 ≠ 理解**；成本按时长线性增长 | 进阶 | Transformer 架构 |
| [扩散模型与图像生成](../concepts/ai/diffusion-models.md) | 从噪声反复去噪生成图像。**多步迭代**，因此"步数 ↔ 质量 ↔ 成本"必须显式取舍 | 前沿 | 训练范式总览 |

---

## 走一遍完整链路

把这条链路串成一次真实调用，比逐条背概念有效得多（下面按一次对话的顺序走，多模态输入不在其中）：
...
诚实列出边界，比含糊带过有用。**2026-09-16 更新：多模态与推理时计算已经补齐**（L6 四张 + L3 两张），当前仍缺的是：

- **通用机器学习基础**：反向传播、损失函数、优化器、正则化。**这一条是"明确不收"，不是"待写"**——本库的「AI」定义为 LLM 栈，收录它们等于换一个选材范围。理由与"不收 MySQL / Kubernetes"同源，见[清单](../roadmap.md)的「明确排除」一节。
- **多模态的工程形态**：视频、实时音频流、多模态检索的端到端设计。本轮补的是**基础概念**，工程细节未覆盖。
- **训练与推理的基础设施**：分布式训练、集群调度、硬件选型。库里最深只到「推理引擎与服务化」这一层。

另有一类不是"缺"而是**有意不收**：具体框架、推理服务产品、模型厂商对比。收录原则见 [`maps/software.md`](../maps/software.md) 末尾那句判断标准——"换一个语言或框架之后，这个概念还成立吗？"

**一个自我提醒**：本文上方的「层级分布」与各层卡片数是**活数字**，每次补卡都会变。写在这里是为了让读者知道规模，不是因为它们稳定——**若与 [`concepts/ai/`](../concepts/ai/) 的实际文件数不一致，以文件为准。**
