...
**当前状态：全部 77 张卡片的信源等级都是 `unverified`** —— 内容由 AI 生成、结构完整，但**来源链接尚未逐条打开核对，正文表述也未经改写**。

这意味着现在的正确用法是**当作索引与提问清单**（知道有哪些概念、它们怎么依赖），而不是当作事实依据。核查升级路径见 [`roadmap.md`](roadmap.md) 的「下一步」。
